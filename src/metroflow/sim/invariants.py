...
from metroflow.sim.active_agents import ActiveAgentPool, validate_active_agent_pool
...
def check_non_negative_queue(
    ...
    *,
    ...
    arr = jnp.asarray(queue_values)
    if arr.ndim == 0:
        arr = arr.reshape((1,))
    negative_mask = arr < 0
    if not bool(jnp.any(negative_mask)):
        return ()

    negative_indices = jnp.nonzero(negative_mask, size=arr.size, fill_value=-1)[0]
    first_index = int(negative_indices[0])
    min_value = float(jnp.min(arr))
    negative_count = int(jnp.sum(negative_mask))
    return (
        InvariantViolation(
...
def check_capacity_violation_flags(
    ...
    *,
    ...
    outflow = jnp.asarray(outflow_values, dtype=jnp.float32)
    capacity = jnp.asarray(effective_capacity_values, dtype=jnp.float32)
    ...
    exceed_count = int(jnp.sum(exceed_mask))
...
def validate_invariants(state: SimulationState) -> InvariantReport:
    ...
    if isinstance(pool, ActiveAgentPool):
        for issue in validate_active_agent_pool(pool):
            violations.append(
                InvariantViolation(
...
def _resolve_effective_capacity_values(flow_link_state: Any) -> Any | None:
    ...
    return jnp.asarray(base_capacity, dtype=jnp.float32) * jnp.asarray(
        multiplier,
        dtype=jnp.float32,
    )
