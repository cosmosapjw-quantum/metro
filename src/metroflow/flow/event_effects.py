...
def apply_active_event_effects_to_link_state(
    *,
    ...
    effective_capacity = link_state.capacity_veh_per_tick * multiplier
    next_flags = link_state.outflow_vehicles > (effective_capacity + 1e-6)
    next_link_state = replace(
        link_state,
...
def compute_incident_capacity_multiplier_from_events(
    *,
...
) -> tuple[jnp.ndarray, tuple[int, ...], tuple[int, ...], tuple[int, ...]]:
    ...
    base = jnp.ones((link_count,), dtype=jnp.float32)
    if link_count == 0:
        return base, (), (), ()
...
def _apply_event_multiplier_updates_core(
    *,
    ...
    idx = jnp.asarray(update_indices, dtype=jnp.int32)
    vals = jnp.asarray(update_multipliers, dtype=jnp.float32)
    return base_multiplier.at[idx].set(jnp.minimum(base_multiplier[idx], vals))
