...
def generate_behavior_profiles(
    *,
    ...
    reroute_jitter = [
        float(v)
        for v in jrandom.uniform(
            fold_in_path(key, "behavior_profiles", "reroute_jitter"),
            shape=(count,),
            minval=-0.05,
            maxval=0.05,
        ).tolist()
    ...
    explore_noise = [
        float(v)
        for v in jrandom.uniform(
            fold_in_path(key, "behavior_profiles", "explore_noise"),
            shape=(count,),
            minval=0.0,
            maxval=1.0,
        ).tolist()
...
def _plan_citizen_population_layout(
    *,
    ...
    worker_order = (
        tuple(
            int(v)
            for v in jrandom.permutation(
                fold_in_path(key, "population", "worker_order"),
                citizen_count,
            ).tolist()
        )
        if citizen_count
...
def _build_leisure_pref_cache(
    ...
    *,
    ...
    if not pool:
        return ()
    n = min(3, len(pool))
    weight_rows = jrandom.uniform(
        fold_in_path(key_from_seed(seed), "population", "leisure_pref_weights"),
        shape=(len(pool), n),
        minval=0.5,
        maxval=1.5,
    ).tolist()
