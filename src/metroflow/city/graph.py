...
class RoadNetworkCSR:
    ...
    def __post_init__(self) -> None:
        ...
        self.node_ids = _as_array(self.node_ids, [node.node_id for node in self.nodes], jnp.int32)
        self.link_ids = _as_array(self.link_ids, [link.link_id for link in self.links], jnp.int32)
        ...
        out_indptr, out_indices = _build_link_csr(
            ...
            node_indices=list(jnp.asarray(self.link_src_node_index).tolist()),
        )
        in_indptr, in_indices = _build_link_csr(
            node_count=len(self.nodes),
            node_indices=list(jnp.asarray(self.link_dst_node_index).tolist()),
        )
        self.outgoing_indptr = _as_array(self.outgoing_indptr, out_indptr, jnp.int32)
        self.outgoing_link_indices = _as_array(self.outgoing_link_indices, out_indices, jnp.int32)
        self.incoming_indptr = _as_array(self.incoming_indptr, in_indptr, jnp.int32)
        self.incoming_link_indices = _as_array(self.incoming_link_indices, in_indices, jnp.int32)
    ...
    def outgoing_links_for_node(self, node_id: int) -> tuple[RoadLink, ...]:
        ...
        indices = [int(x) for x in jnp.asarray(self.outgoing_link_indices[start:end]).tolist()]
...
def _as_array(value: Array | None, fallback: Iterable[Any], dtype: Any) -> Array:
    if value is None:
        return jnp.asarray(list(fallback), dtype=dtype)
