...
class ODBanditState:
    ...
    def __post_init__(self) -> None:
        ...
        self.pull_count = jnp.asarray(self.pull_count, dtype=jnp.int32)
        self.estimated_reward = jnp.asarray(self.estimated_reward, dtype=jnp.float32)
        if self.candidate_ids is not None:
            self.candidate_ids = jnp.asarray(self.candidate_ids, dtype=jnp.int32)
        ...
        if bool(jnp.any(self.pull_count < 0)):
            raise ValueError("pull_count values must be >= 0")
        if not bool(jnp.all(jnp.isfinite(self.estimated_reward))):
        ...
        if self.exploration_bonus is None:
            self.exploration_bonus = jnp.zeros((self.arm_count,), dtype=jnp.float32)
        else:
            self.exploration_bonus = jnp.asarray(self.exploration_bonus, dtype=jnp.float32)
            ...
            if not bool(jnp.all(jnp.isfinite(self.exploration_bonus))):
...
def create_od_ucb_state(
    *,
    ...
    pulls = jnp.asarray(pull_count if pull_count is not None else (0,) * n, dtype=jnp.int32)
    rewards = jnp.asarray(
    ...
    if candidate_ids is None:
        candidate_ids = jnp.arange(n, dtype=jnp.int32)
...
def compute_ucb_scores_core(
    ...
    *,
    ...
    """Array-only UCB1 scores for future JIT-friendly call sites."""

    est = jnp.asarray(estimated_reward, dtype=jnp.float32)
    pulls = jnp.asarray(pull_count, dtype=jnp.float32)
    total_pulls = jnp.maximum(1.0, jnp.sum(pulls))
    safe_pulls = jnp.maximum(1.0, pulls)
    scale = jnp.asarray(exploration_scale, dtype=jnp.float32)
    bonus = scale * jnp.sqrt(jnp.log(total_pulls + 1.0) / safe_pulls)
    # Prefer any unpulled arm deterministically by giving it a large finite score.
    unpulled_mask = pulls <= 0.0
    return jnp.where(unpulled_mask, jnp.asarray(1.0e9, dtype=jnp.float32), est + bonus)
...
def update_od_ucb_arrays_core(
    ...
    *,
    ...
    pulls = jnp.asarray(pull_count, dtype=jnp.int32)
    rewards = jnp.asarray(estimated_reward, dtype=jnp.float32)
    idx = jnp.asarray(arm_index, dtype=jnp.int32)
    reward_value = jnp.asarray(reward, dtype=jnp.float32)

    next_pulls = pulls.at[idx].add(1)
    n = jnp.asarray(next_pulls[idx], dtype=jnp.float32)
    prev_mean = rewards[idx]
    next_rewards = rewards.at[idx].set(prev_mean + (reward_value - prev_mean) / jnp.maximum(1.0, n))
    scores = compute_ucb_scores_core(next_rewards, next_pulls)
    unpulled = next_pulls <= 0
    next_bonus = jnp.where(unpulled, jnp.asarray(1.0e9, dtype=jnp.float32), scores - next_rewards)
...
def update_od_ucb_state(
    *,
    ...
    reward_value = jnp.asarray(reward, dtype=jnp.float32)
    if not bool(jnp.isfinite(reward_value)):
        raise ValueError("reward must be finite")

    next_pulls_arr, next_rewards_arr, next_bonuses_arr = update_od_ucb_arrays_core(
        state.pull_count,
        state.estimated_reward,
        arm_index=idx,
        reward=reward_value,
