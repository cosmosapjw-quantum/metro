...
def compute_reroute_trigger_score_core(
    *,
    ...
    """Array-friendly trigger-score core for future batched/JIT evaluation."""

    reroute = jnp.clip(jnp.asarray(reroute_willingness, dtype=jnp.float32), 0.0, 1.0)
    delay = jnp.asarray(delay_sensitivity, dtype=jnp.float32)
    explore = jnp.clip(jnp.asarray(exploration_bias, dtype=jnp.float32), 0.0, 1.0)
    improvement = jnp.maximum(0.0, jnp.asarray(improvement_ratio, dtype=jnp.float32))
    delay_norm = jnp.where(delay > 0.0, delay / (1.0 + delay), 0.0)
    return (
        0.45 * reroute
        + 0.35 * jnp.minimum(1.0, improvement * (0.75 + 0.75 * delay_norm))
        + 0.20 * explore
    )
...
def compute_reroute_decision_core(
    *,
    ...
    persist = jnp.asarray(persistence_bias, dtype=jnp.float32)
    resistance = 0.40 + 0.15 * jnp.where(jnp.abs(persist) > 0.0, jnp.abs(persist) / (1.0 + jnp.abs(persist)), 0.0)
    should = jnp.logical_and(score >= resistance, jnp.asarray(reroute_willingness, dtype=jnp.float32) > 0.0)
    return should, score
